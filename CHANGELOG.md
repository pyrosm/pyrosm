...
- NEW: Add a `keep_metadata` parameter to `OSM(...)` (default `True`). Set `keep_metadata=False` to drop the way/relation element metadata columns (`timestamp`, `version`, `changeset`) from the returned GeoDataFrames, reducing memory use; the default behaviour is unchanged and node-level metadata is unaffected (#87)
