...
- NEW: Add `pyrosm.get_data_by_bbox(bbox, url=True, update=False)` to suggest which Geofabrik extract to download for a bounding box. It returns the smallest Geofabrik extract whose extent fully covers the box — its PBF download URL by default, or its Geofabrik id with `url=False` — and prints the matched extract's name. The bounding box may be a `[minx, miny, maxx, maxy]` list/tuple/array in lon/lat, a Shapely geometry, or a GeoDataFrame/GeoSeries. It is backed by a vendored snapshot of Geofabrik's `index-v1.json`, so suggestions work offline (refresh it with `scripts/update_geofabrik_index.py`); `update=True` fetches the live index instead (#165, #197)
- NEW: Add `pyrosm.geocode(query)` and `pyrosm.get_data_by_geocoding(query, crop=False)` to fetch data by place name. `geocode` returns a Shapely polygon for a place (e.g. `"Brighton and Hove, UK"`) via OpenStreetMap's Nominatim service — its boundary polygon when available, otherwise its bounding-box rectangle. `get_data_by_geocoding` geocodes the place, finds the smallest Geofabrik extract that covers it (`get_data_by_bbox`), downloads it and returns the local path; with `crop=True` it crops the downloaded extract to the geocoded area first. No new dependencies (stdlib `urllib`/`json` with the bundled `certifi`, and `shapely`) (#165)
